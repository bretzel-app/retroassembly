...
- New `top100_ranks` database table and migration (`0007_top100_ranking`; renumbered
  from `0005` after rebasing on upstream, which took `0005` and `0006`. The SQL uses
  `IF NOT EXISTS` so deployments that already ran the old `0005_top100_ranking`
  migration re-apply it harmlessly under the new number)
...
- Last rebased on upstream `main` at commit `2b4fa21` ("fix: fix cf deploy", 2026-07)
  — 29 upstream commits including the react-router v8 upgrade, per-user library
  modes (`effectiveLibraryUserId`), N64 state saving, media carousel, core options,
  and database index optimizations.
